...
def send_selection():
    global cursor, gameMap, own_turn, p1_turn, p2_turn
    xCurs = cursor[0]
    yCurs = cursor[1]
    zCurs = cursor[2]
    if gameMap[xCurs][yCurs][zCurs] != 0:
        ...
        xCursPlus = min(4, max(0,xCurs+1))
        xCursMinus = min(4, max(0,xCurs-1))
        yCursPlus = min(4, max(0,yCurs+1))
        yCursMinus = min(4, max(0,yCurs-1))
        zCursPlus = min(4, max(0,zCurs+1))
        zCursMinus = min(4, max(0,zCurs-1))
        
        neighbors=[]
        neighbors.append(gameMap[xCursPlus][yCurs][zCurs])
        neighbors.append(gameMap[xCursMinus][yCurs][zCurs])
        neighbors.append(gameMap[xCurs][yCursPlus][zCurs])
        neighbors.append(gameMap[xCurs][yCursMinus][zCurs])
        neighbors.append(gameMap[xCurs][yCurs][zCursPlus])
        neighbors.append(gameMap[xCurs][yCurs][zCursMinus])
        
        for i in range(0,len(neighbors)):
            if not neighbors[i] in [-1,0,player_ID]:
                if i == 0:
                    gameMap[xCursPlus][yCurs][zCurs] = player_ID
                    bt_send([xCursPlus,yCurs,zCurs,player_ID])
                elif i == 1:
                    gameMap[xCursMinus][yCurs][zCurs] = player_ID
                    bt_send([xCursMinus,yCurs,zCurs,player_ID])
                elif i == 2:
                    gameMap[xCurs][yCursPlus][zCurs] = player_ID
                    bt_send([xCurs,yCursPlus,zCurs,player_ID])
                elif i == 3:
                    gameMap[xCurs][yCursMinus][zCurs] = player_ID
                    bt_send([xCurs,yCursMinus,zCurs,player_ID])
                elif i == 4:
                    gameMap[xCurs][yCurs][zCursPlus] = player_ID
                    bt_send([xCursMinus,yCurs,zCursPlus,player_ID])
                elif i == 5:
                    gameMap[xCurs][yCurs][zCursMinus] = player_ID
                    bt_send([xCurs,yCurs,zCursMinus,player_ID])
...
while True:
    ...
    if p1_turn or p2_turn:
        #gameloop
        while (p1_turn or p2_turn) and move_count < 97:
            if not button.value():
                start_time = time()
...
#                 move_count += 1
        ...
        elif re.match(re.compile("...1"),gameVar): #change for other player ##TODO
            p2_turn = True
            player_ID = 2
            #own_turn = True
#             if move_count > 0:
